...
class BottleneckBlock(nn.Module):
    def __init__(
        ...
        self.conv1 = ConvBlock(
            in_channels, (in_channels + out_channels) // 2, kernel_size, padding=1
        )
        self.conv2 = ConvBlock(
            (in_channels + out_channels) // 2, out_channels, kernel_size, padding=1
        )
...
class C2f(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = ConvBlock(
            in_channels, self.hidden_channels * 2, kernel_size=kernel_size, stride=1
        )
...
class SPPF(nn.Module):
    ...
    def __init__(
        self, in_channels: int, out_channels: int, maxpool_kernel_size=5
        ...
        self.max_pool = nn.MaxPool2d(
            kernel_size=maxpool_kernel_size, stride=1, padding=maxpool_kernel_size // 2
        )
