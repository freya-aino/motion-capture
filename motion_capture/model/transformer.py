...
class TransformerEncoderBlock(nn.Module):
    def __init__(self, input_dim: int, output_dim: int, depth: int = 1):
        ...
        self.residual_1 = nn.Sequential(nn.Linear(input_dim, input_dim), nn.ELU())
        self.residual_2 = nn.Sequential(nn.Linear(input_dim, output_dim), nn.ELU())
        
    def forward(self, x: T.Tensor, return_attention_matrix: bool = False):
        
        y1, attention_matrix = self.MHA(x, x, x)
...
class TransformerDecoderBlock(nn.Module):
    ...
    def forward(self, x: T.Tensor, memory: T.Tensor, return_attention_matrix: bool = False):
