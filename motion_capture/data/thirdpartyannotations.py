...
def anno_wrapper(func):
    def wrap(*args, **kwargs):
        ...
        local_anno_path = os.path.join(
            ANNOTATION_PATH, kwargs["service_name"], kwargs["video_name"]
        )
...
def openpose_annotate(video_name, service_name):
    ...
    proj_path = (
        Path(__file__).absolute().parent.joinpath("external_repo_cache", "openpose")
    )
    ...
    for cam_name in os.listdir(os.path.join(VIDEO_PATH, video_name)):
        ...
        for ji in os.listdir(os.path.join(local_anno_path, cam_id)):
            ...
            if "people" in J and len(J["people"]) > 0:
                pose_keypoints_2d = (
                    np.array(J["people"][0]["pose_keypoints_2d"]).reshape(25, 3)
                    if "pose_keypoints_2d" in J["people"][0]
                    else None
                )
                face_keypoints_2d = (
                    np.array(J["people"][0]["face_keypoints_2d"]).reshape(70, 3)
                    if "face_keypoints_2d" in J["people"][0]
                    else None
                )
                hand_left_keypoints_2d = (
                    np.array(J["people"][0]["hand_left_keypoints_2d"]).reshape(21, 3)
                    if "hand_left_keypoints_2d" in J["people"][0]
                    else None
                )
                hand_right_keypoints_2d = (
                    np.array(J["people"][0]["hand_right_keypoints_2d"]).reshape(21, 3)
                    if "hand_right_keypoints_2d" in J["people"][0]
                    else None
                )

                pose_keypoints_2d, pose_scores_2d = (
                    (
                        pose_keypoints_2d[:, :2].flatten(),
                        pose_keypoints_2d[:, 2].flatten(),
                    )
                    if type(pose_keypoints_2d) == np.ndarray
                    else (None, None)
                )
                face_keypoints_2d, face_scores_2d = (
                    (
                        face_keypoints_2d[:, :2].flatten(),
                        face_keypoints_2d[:, 2].flatten(),
                    )
                    if type(face_keypoints_2d) == np.ndarray
                    else (None, None)
                )
                hand_left_keypoints_2d, hand_left_scores_2d = (
                    (
                        hand_left_keypoints_2d[:, :2].flatten(),
                        hand_left_keypoints_2d[:, 2].flatten(),
                    )
                    if type(hand_left_keypoints_2d) == np.ndarray
                    else (None, None)
                )
                hand_right_keypoints_2d, hand_right_scores_2d = (
                    (
                        hand_right_keypoints_2d[:, :2].flatten(),
                        hand_right_keypoints_2d[:, 2].flatten(),
                    )
                    if type(hand_right_keypoints_2d) == np.ndarray
                    else (None, None)
                )
        ...
        out = pl.DataFrame.from_records(out)
        fit_table = pd.DataFrame.from_records(
            [{"Frame": i} for i in range(max(out["Frame"]))]
        )
...
def prepare_op(x):
    availability = T.tensor(
        reduce(lambda a, b: a & b, [~x[k].isna() for k in x.keys()]), dtype=T.bool
    )

    keypoints_pose = T.stack(
        [
            T.tensor(list(x["Keypoints_pose"].iloc[i])).reshape(25, 2)
            if availability[i]
            else T.nan * T.ones(25, 2)
            for i in range(len(x))
        ]
    ).to(dtype=T.float32)
    keypoints_face = T.stack(
        [
            T.tensor(list(x["Keypoints_face"].iloc[i])).reshape(70, 2)
            if availability[i]
            else T.nan * T.ones(70, 2)
            for i in range(len(x))
        ]
    ).to(dtype=T.float32)
    keypoints_left_hand = T.stack(
        [
            T.tensor(list(x["Keypoints_left_hand"].iloc[i])).reshape(21, 2)
            if availability[i]
            else T.nan * T.ones(21, 2)
            for i in range(len(x))
        ]
    ).to(dtype=T.float32)
    keypoints_right_hand = T.stack(
        [
            T.tensor(list(x["Keypoints_right_hand"].iloc[i])).reshape(21, 2)
            if availability[i]
            else T.nan * T.ones(21, 2)
            for i in range(len(x))
        ]
    ...
    scores_pose = T.stack(
        [
            T.tensor(list(x["Scores_pose"].iloc[i]))
            if availability[i]
            else T.nan * T.ones(25)
            for i in range(len(x))
        ]
    ).to(dtype=T.float32)
    scores_face = T.stack(
        [
            T.tensor(list(x["Scores_face"].iloc[i]))
            if availability[i]
            else T.nan * T.ones(70)
            for i in range(len(x))
        ]
    ).to(dtype=T.float32)
    scores_left_hand = T.stack(
        [
            T.tensor(list(x["Scores_left_hand"].iloc[i]))
            if availability[i]
            else T.nan * T.ones(21)
            for i in range(len(x))
        ]
    ).to(dtype=T.float32)
    scores_right_hand = T.stack(
        [
            T.tensor(list(x["Scores_right_hand"].iloc[i]))
            if availability[i]
            else T.nan * T.ones(21)
            for i in range(len(x))
        ]
    ...
    visibilities_pose = ~(keypoints_pose == 0.0).all(-1)
    visibilities_face = ~(keypoints_face == 0.0).all(-1)
    visibilities_left_hand = ~(keypoints_left_hand == 0.0).all(-1)
    visibilities_right_hand = ~(keypoints_right_hand == 0.0).all(-1)
...
def open_see_face_annotate(video_name, service_name):
    ...
    proj_path = (
        Path(__file__)
        .absolute()
        .parent.joinpath("external_repo_cache", "OpenSeeFace_altered")
    )
    sys.path.append(str(proj_path))

    from external_repo_cache.OpenSeeFace_altered.tracker import (
        Tracker as open_see_face_tracker,
    )
...
def prepare_osf(x, face_anchor_offset=0.6):
    x.loc[x["Success"].isna(), "Success"] = 0.0
    x["Success"] = x["Success"].astype(bool)

    availability = T.tensor(
        reduce(lambda a, b: a & b, [x["Success"], *[~x[k].isna() for k in x.keys()]]),
        dtype=T.bool,
    )

    kpts = T.stack(
        [
            T.tensor(list(x["Keypoints"].iloc[i])).reshape(68, 3)
            if availability[i]
            else T.nan * T.ones(68, 3)
            for i in range(len(x))
        ]
    ).to(dtype=T.float32)
    ...
    kpts_3d_image_space = T.stack(
        [
            T.tensor(list(x["Keypoints_3D"].iloc[i])).reshape(70, 3)
            if availability[i]
            else T.nan * T.ones(70, 3)
            for i in range(len(x))
        ]
    ).to(dtype=T.float32)

    static_face_ancor = kpts_3d_image_space[:, 27:36, :].nanmean(
        1, keepdim=True
    ) - T.tensor([0, 0, face_anchor_offset])
    ...
    translation = T.stack(
        [
            T.tensor(list(x["Translation"].iloc[i]))
            if availability[i]
            else T.nan * T.ones(3)
            for i in range(len(x))
        ]
    ...
    quaternion = T.stack(
        [
            T.tensor(list(x["Quaternion"].iloc[i]))
            if availability[i]
            else T.nan * T.ones(4)
            for i in range(len(x))
        ]
    ...
    camera_rot = np.array(
        [
            [CAMERA_OUT_WIDTH, 0, CAMERA_OUT_WIDTH / 2],
            [0, CAMERA_OUT_WIDTH, CAMERA_OUT_HEIGHT / 2],
            [0, 0, 1],
        ],
        dtype=np.float32,
    )
    ...
    return {
        "availability": availability,
        "confidences": T.tensor(
            [x["Conf"].iloc[i] if availability[i] else T.nan for i in range(len(x))]
        ).to(dtype=T.float32),
...
def osf_kpts_prep(osf_image_space, osf_local_space, icp_max_iters=10, icp_samples=30):
    ...
    if osf_valid_mask.any():
        osf_ln[osf_valid_mask] = T.tensor(
            stochasticICP_search(
                osf_ln[osf_valid_mask].numpy().T,
                osf_gn[osf_valid_mask].numpy().T,
                icp_max_iters,
                icp_samples,
            ),
            dtype=T.float32,
