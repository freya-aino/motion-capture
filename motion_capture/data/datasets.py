...
class CelebA(data.Dataset):
    def __init__(self, annotatin_path, image_path, image_shape_WH):
        ...
        landmarks = pd.read_csv(
            os.path.join(annotatin_path, "list_landmarks_celeba.txt"),
            header=1,
            delimiter=r"\s+",
        )
        # attributes = pd.read_csv(os.path.join(annotatin_path, "list_attr_celeba.txt"), header=1, delimiter=r"\s+")
        bboxes = pd.read_csv(
            os.path.join(annotatin_path, "list_bbox_celeba.txt"),
            header=1,
            delimiter=r"\s+",
        )
        ...
        self.all_datapoints["imagePath"] = (
            image_path + "/" + self.all_datapoints["image_id"]
        )
...
class WIDERFace(data.Dataset):
    def __init__(self, path: str, image_shape_WH: tuple, max_number_of_faces: int):
        ...
        train_annotations = pd.read_csv(
            os.path.join(path, "train", "train_bbx_gt.txt"), header=None
        )[0]
        val_annotations = pd.read_csv(
            os.path.join(path, "val", "val_bbx_gt.txt"), header=None
        )[0]
        ...
        annotations[train_file_mask] = (
            train_image_path + "/" + annotations[train_file_mask]
        )
    ...
    def __getitem__(self, idx):
        ...
        blur = pad(
            one_hot(faces[:, 4], 3),
            (0, 0, 0, self.max_number_of_faces - num_faces),
            value=0,
        )
        typical_expression = pad(
            one_hot(faces[:, 5], 2),
            (0, 0, 0, self.max_number_of_faces - num_faces),
            value=0,
        )
        normal_illumination = pad(
            one_hot(faces[:, 6], 2),
            (0, 0, 0, self.max_number_of_faces - num_faces),
            value=0,
        )
...
class WFLW(data.Dataset):
    def __init__(self, image_shape_WH: tuple, path: str, max_number_of_faces: int):
        ...
        train_datapoints = pd.read_csv(
            os.path.join(path, "annotations", "train.txt"), header=None
        )[0]
        validation_datapoints = pd.read_csv(
            os.path.join(path, "annotations", "validation.txt"), header=None
        )[0]

        datapoints = pd.concat(
            [train_datapoints, validation_datapoints], ignore_index=True
        )
        datapoints = datapoints.map(lambda x: x.split(" "))

        self.all_datapoints = pd.DataFrame()
        self.all_datapoints["imagePath"] = datapoints.map(
            lambda x: os.path.join(image_folder_path, x[-1])
        )
        self.all_datapoints["bbox"] = datapoints.map(
            lambda x: np.array(x[196:200], dtype=np.int16).reshape(2, 2)
        )
        self.all_datapoints["keypoints"] = datapoints.map(
            lambda x: np.array(x[:196], dtype=np.float32).reshape(-1, 2)
        )
        self.all_datapoints["indicators"] = datapoints.map(
            lambda x: np.array(x[200:206], dtype=np.int16)
        )

        self.all_datapoints = [
            (image_path, {k: list(v.values()) for k, v in elements.to_dict().items()})
            for image_path, elements in self.all_datapoints.groupby("imagePath")
            .filter(lambda x: len(x["bbox"]) <= self.max_number_of_faces)
            .groupby("imagePath")
        ]
    ...
    def __getitem__(self, idx):
        ...
        bboxes = pad(
            bboxes[: self.max_number_of_faces].reshape(-1, 4),
            (0, 0, 0, max(0, self.max_number_of_faces - len(bboxes))),
            value=0,
        )
        ...
        keypoints = scale_points(keypoints, image.shape[::-1][:2], [1, 1])
        keypoints = pad(
            keypoints[: self.max_number_of_faces],
            (0, 0, 0, max(0, self.max_number_of_faces - len(keypoints))),
            value=0,
        )
        ...
        pose = pad(
            one_hot(indicators[:, 0], 2),
            (0, 0, 0, max(0, self.max_number_of_faces - len(indicators))),
            value=0,
        )
        expression = pad(
            one_hot(indicators[:, 1], 2),
            (0, 0, 0, max(0, self.max_number_of_faces - len(indicators))),
            value=0,
        )
        illumination = pad(
            one_hot(indicators[:, 2], 2),
            (0, 0, 0, max(0, self.max_number_of_faces - len(indicators))),
            value=0,
        )
        make_up = pad(
            one_hot(indicators[:, 3], 2),
            (0, 0, 0, max(0, self.max_number_of_faces - len(indicators))),
            value=0,
        )
        occlusion = pad(
            one_hot(indicators[:, 4], 2),
            (0, 0, 0, max(0, self.max_number_of_faces - len(indicators))),
            value=0,
        )
        blur = pad(
            one_hot(indicators[:, 5], 2),
            (0, 0, 0, max(0, self.max_number_of_faces - len(indicators))),
            value=0,
        )
...
class MPIIDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = read_image(
            os.path.join(self.image_folder_path, dp["image"]), mode=ImageReadMode.RGB
        ).to(dtype=T.float32)
        ...
        bbox = T.stack(
            [visible_keypoints.min(dim=0)[0], visible_keypoints.max(dim=0)[0]]
        )
        bbox = T.cat([bbox[0], bbox[1] - bbox[0]]).to(dtype=T.int16).reshape(2, 2)

        # scale bbox and keypoints to full image size
        full_scaled_bbox = scale_points(
            bbox, image.shape[::-1][:2], self.output_full_image_shape
        )
        full_scaled_keypoints = scale_points(
            keypoints, image.shape[::-1][:2], self.output_full_image_shape
        )
        ...
        local_scaled_keypoints = scale_points(
            keypoints - bbox[0],
            person_image.shape[::-1][:2],
            self.output_person_image_shape,
        )
...
class COCO2017GlobalPersonInstanceSegmentation(data.Dataset):
    def __init__(
        ...
        with open(
            os.path.join(annotation_folder_path, "person_keypoints_train2017.json"), "r"
        ) as f:
            train_json = json.load(f)
        with open(
            os.path.join(annotation_folder_path, "person_keypoints_val2017.json"), "r"
        ) as f:
            val_json = json.load(f)

        images = pd.DataFrame.from_records((*train_json["images"], *val_json["images"]))
        annotations = pd.DataFrame.from_records(
            (*train_json["annotations"], *val_json["annotations"])
        )
        ...
        self.all_datapoints["image_path"] = (
            image_folder_path + "/" + self.all_datapoints["file_name"]
        )
        self.all_datapoints = self.all_datapoints[
            self.all_datapoints["bbox"].apply(
                lambda x: x[2] * x[3] > (min_bbox_size**2)
            )
        ]
        self.all_datapoints = self.all_datapoints[
            self.all_datapoints["segmentation"].apply(type) == list
        ]

        self.all_datapoints.drop(
            columns=[
                "num_keypoints",
                "area",
                "iscrowd",
    ...
    def __getitem__(self, idx):
        ...
        bboxes = T.tensor(
            bboxes, dtype=T.float32
        ).reshape(
            -1, 2, 2
        )  # ([scale_points(T.tensor(bbox).reshape(2, 2), self.image_shape[::-1], [1, 1]) for bbox in bboxes])
        bboxes = scale_points(bboxes, image.shape[::-1][:2], [1, 1])
        bboxes[:, 1, :] += bboxes[:, 0, :]
        ...
        for seg in segmentations:
            ...
            seg = pad(
                seg,
                (0, 0, 0, max(0, self.max_segmentation_points - seg.shape[0])),
                value=0,
            )
...
class COCO2017PersonKeypointsDataset(data.Dataset):
    def __init__(
        ...
        self.all_datapoints = pd.merge(
            left=annotations, right=images, left_on="image_id", right_on="id"
        )

        # filter on bbox size
        self.all_datapoints = self.all_datapoints[
            self.all_datapoints["bbox"].apply(
                lambda x: x[2] > self.min_person_bbox_size
                and x[3] > self.min_person_bbox_size
            )
        ]
    ...
    def __getitem__(self, idx):
        ...
        keypoints = scale_points(
            keypoints[:, :2] - bbox[0], person_crop.shape[::-1][:2], [1, 1]
        )
...
class COCOPanopticsObjectDetection(data.Dataset):
    def __init__(
        ...
        self.images = pd.DataFrame.from_records(
            (*train_json["images"], *val_json["images"])
        )

        # filter on number of segments
        num_segments = annotations["segments_info"].map(len)
        annotations = annotations[
            (num_segments > 0) & (num_segments <= self.max_number_of_instances)
        ]

        # format segment info
        annotations["bboxes"] = annotations["segments_info"].map(
            lambda x: np.array([xx["bbox"] for xx in x])
        )
        annotations["categoryIds"] = annotations["segments_info"].map(
            lambda x: np.array([xx["category_id"] for xx in x])
        )
        annotations["file_name"] = annotations["file_name"].map(
            lambda x: f"{x[:-4]}.jpg"
        )
    ...
    def __getitem__(self, idx):
        ...
        bboxes = pad(
            bboxes,
            (0, 0, 0, max(0, self.max_number_of_instances - len(bboxes))),
            value=0,
        )
        categories = pad(
            categories,
            (0, 0, 0, max(0, self.max_number_of_instances - len(categories))),
            value=0,
        )

        image = resize(image, self.image_shape[::-1]) / 255

        return image, {"validity": validity, "bboxes": bboxes, "categories": categories}
...
class COCO2017PersonWholeBody(data.Dataset):
    def __init__(
        ...
        with open(
            os.path.join(annotations_folder_path, "coco_wholebody_val_v1.0.json"), "r"
        ) as f:
            val_json = json.load(f)
        with open(
            os.path.join(annotations_folder_path, "coco_wholebody_train_v1.0.json"), "r"
        ) as f:
            train_json = json.load(f)
        ...
        self.all_datapoints = pd.merge(
            annotations, images, right_on="id", left_on="image_id"
        )
        self.all_datapoints["image_path"] = (
            self.image_folder_path + "/" + self.all_datapoints["file_name"]
        )
        self.all_datapoints = self.all_datapoints[
            self.all_datapoints["bbox"].map(lambda x: x[2] * x[3] > area)
        ]

        validity_mask = (
            (self.all_datapoints["num_keypoints"] != 0)
            | self.all_datapoints["face_valid"]
            | self.all_datapoints["lefthand_valid"]
            | self.all_datapoints["righthand_valid"]
            | self.all_datapoints["foot_valid"]
        )
    ...
    def __getitem__(self, idx):
        ...
        # load image
        image = read_image(datapoint["image_path"], mode=ImageReadMode.RGB).to(
            dtype=T.float32
        )
        person_bbox = T.tensor(datapoint["bbox"], dtype=T.int16).reshape(2, 2)
        face_bbox = T.tensor(datapoint["face_box"], dtype=T.int16).reshape(2, 2)
        lefthand_bbox = T.tensor(datapoint["lefthand_box"], dtype=T.int16).reshape(2, 2)
        righthand_bbox = T.tensor(datapoint["righthand_box"], dtype=T.int16).reshape(
            2, 2
        )
        ...
        # scale keypoints
        all_keypoints = scale_points(
            all_keypoints - person_bbox[0], person_bbox[1], [1, 1]
        )

        # scale bounding boxes
        face_bbox = scale_points(
            face_bbox.to(dtype=T.float32) - T.stack([person_bbox[0], T.zeros(2)]),
            person_bbox[1],
            [1, 1],
        )
        lefthand_bbox = scale_points(
            lefthand_bbox.to(dtype=T.float32) - T.stack([person_bbox[0], T.zeros(2)]),
            person_bbox[1],
            [1, 1],
        )
        righthand_bbox = scale_points(
            righthand_bbox.to(dtype=T.float32) - T.stack([person_bbox[0], T.zeros(2)]),
            person_bbox[1],
            [1, 1],
        )
...
class HAKELarge(data.Dataset):
    def __init__(self, annotation_path, image_path, image_shape_WH, max_num_bboxes=10):
        ...
        with open(
            os.path.join(annotation_path, "hake_large_annotation.json"), "r"
        ) as f:
    ...
    def format_datapoint(self, k, v):
        ...
        return {
            ...
            "humanBboxes": T.stack([T.tensor(lab["human_bbox"]) for lab in v["labels"]])
            .to(dtype=T.float32)
            .reshape(-1, 2, 2),
            "objectBboxes": T.stack(
                [T.tensor(lab["object_bbox"]) for lab in v["labels"]]
            )
            .to(dtype=T.float32)
            .reshape(-1, 2, 2),
            # "actionLabels": [{
    ...
    def __getitem__(self, idx):
        ...
        human_bboxes = pad(
            human_bboxes,
            (0, 0, 0, max(0, self.max_num_bboxes - human_bboxes.shape[0])),
            value=0,
        )
        ...
        object_bboxes = pad(
            object_bboxes,
            (0, 0, 0, max(0, self.max_num_bboxes - object_bboxes.shape[0])),
            value=0,
        )

        image = resize(image, self.image_shape[::-1])

        return image, dp, {"humanBboxes": human_bboxes, "objectBboxes": object_bboxes}
