...
#     # ---------------------------------------------------------------------------------------------------------------
#     print("load experiments from config ...")
...
def run(cfg: DictConfig):
    ...
    model_cfg = dict(experiment.model)
    model_training_cfg = dict(experiment.model_training)
    data_cfg = dict(experiment.data)
    
    # parse str attributes as python attributes
    model_class = getattr(models, experiment.model.model_class)
    model_training_cfg["loss_fn"] = getattr(T.nn.functional, experiment.model_training.loss_fn)
    model_training_cfg["optimizer"] = getattr(T.optim, experiment.model_training.optimizer)
    model_training_cfg["lr_scheduler"] = getattr(T.optim.lr_scheduler, experiment.model_training.lr_scheduler)
    data_cfg["dataset_class"] = getattr(datasets, experiment.data.dataset_class)
    data_cfg["image_pertubator"] = preprocessing.ImagePertubators[experiment.data.image_pertubator]
    
    # remove unessesary keys
    model_cfg.pop("model_class")
    ...
    swa = StochasticWeightAveraging(**experiment.stochastic_weight_averaging)
    # ---------------------------------------------------------------------------------------------------------------
    print("initializing trainer ...")
    trainer = pl.Trainer(**experiment.trainer, logger = logger, callbacks = [checkpoint_callback, swa])
    # ---------------------------------------------------------------------------------------------------------------
    print("initialize model ...")
    ...
    # ---------------------------------------------------------------------------------------------------------------
    print("initialize dataset ...")
    data_module = BboxDataModule(**data_cfg)
    data_module.setup()
    # ---------------------------------------------------------------------------------------------------------------
    print("fitting model ...")
    trainer.fit(
